...
public class PasswordRepository {
	...
	public Password findOneById(String id) throws Exception {
		ViewQuery query = ViewQuery.from("password", "by_id").inclusiveEnd(true).key(id);//.stale(Stale.FALSE);
	...
	public Password findOneByOwnerId(String id) throws Exception {
		ViewQuery query = ViewQuery.from("password", "by_owner_id").inclusiveEnd(true).key(id);//.stale(Stale.FALSE);
		ViewResult result = bucket.query(query/*.key(name).limit(10)*/);
		if(!result.success()){
			String error = result.error().toString();
			logger.error("error from view query:" + error);
