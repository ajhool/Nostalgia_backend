...
public class MediaCollectionRepository {
	...
	public List<MediaCollection> findByName(String name) throws Exception {
		ViewQuery query = ViewQuery.from("mediaCollection_standard", "by_name").inclusiveEnd(true).key(name);//.stale(Stale.FALSE);
	...
	public HashSet<MediaCollection> findByChannel(String channel) {
		...
		ViewQuery query = ViewQuery.from("mediaCollection_standard", "by_channel").inclusiveEnd(true).key(channel);//.stale(Stale.FALSE);
		ViewResult result = bucket.query(query/*.key(name).limit(10)*/);
		if(!result.success()){
			String error = result.error().toString();
			logger.error("error from view query:" + error);
	...
	public HashSet<MediaCollection> findByTag(String tag) {
		...
		ViewQuery query = ViewQuery.from("mediaCollection_standard", "by_tag").inclusiveEnd(true).key(tag);//.stale(Stale.FALSE);
		ViewResult result = bucket.query(query/*.key(name).limit(10)*/);
		if(!result.success()){
			String error = result.error().toString();
			logger.error("error from view query:" + error);
